"""
train.py — Fine-tune TrOCR on IAM Handwriting Dataset.

NOTE: MPS (Apple Silicon) is NOT supported for training due to a PyTorch
backward-pass incompatibility with TrOCR's attention layers. This script
uses CUDA if available, otherwise falls back to CPU.
For MPS machines, use Google Colab (free T4 GPU) for training.

Can be run standalone: python src/train.py
"""
...
def train_model(num_train=500, num_val=100, epochs=3, batch_size=4, lr=5e-5,
    ...
    # MPS not supported for training — use CUDA or CPU only
    ...
    ax.plot(range(1, epochs+1), train_losses, "o-", label="Train", color="steelblue")
    ax.plot(range(1, epochs+1), val_losses, "o-", label="Val", color="coral")
    ax.set_xlabel("Epoch"); ax.set_ylabel("Loss"); ax.set_title("Loss Curves"); ax.legend(); ax.grid(alpha=0.3)
