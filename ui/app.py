"""
app.py — ScribeCheck Gradio Interface 
...
def get_confidence_label(score):
    """Map combined score to a human-readable confidence level."""
    if score >= 0.90:
...
def evaluate_handwriting(image, reference_text, w_lev, w_bleu, w_cos):
    ...
    try:
        ...
        preprocessed_pil = cv2_to_pil(preprocessed)
        ...
        return (
            ...
            f"{sim['bleu']:.4f}",
            f"{sim['cosine']:.4f}",
            f"{sim['combined_score']:.4f} — {confidence}",
            quality_str,
        )
